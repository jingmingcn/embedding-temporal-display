...
$(function(){
	...
	var keyword = 'temporal';
	var filter_cluster = 'Ebert, D.S.';
	var cluster;
	...
	d3.csv(url,function(error,mdata){
	    ...
	    mdata.forEach(function(d){
	    	...
	    	if(!abstract || abstract.indexOf(keyword)<0){
	    		return;
	    	}
	...
	var app = function(){
		...
		data.nodes.forEach(function(d,i){
			if(d.id === filter_cluster){
				cluster = d.cluster;
			}
		});
		
		data.nodes = data.nodes.filter(d=>d.cluster==cluster).sort((a,b)=>a.value>b.value);
		data.links = data.links.filter(d=>d.source.cluster==cluster);
		...
		var xScale = d3.scale.linear().range([100,900]).domain([0,data.nodes.length-1]);
		...
		path = svg.selectAll(".area").data(data.links).enter().append('g').attr("class","area").style('z-index',777)
			.each(function(d,i){
		...
		links = svg.selectAll(".link").data(data.links).enter().insert("path").attr("class","link").style('z-index',888);
		...
		content_folder.add(preferences,'keyword').name('Keyword');
