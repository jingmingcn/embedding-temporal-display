...
$(function(){
	...
	d3.csv(url,function(error,mdata){
		...
		console.log('== NetClustering Starts! ==');
		console.time('netClustering');

		netClustering.cluster(data.nodes, data.links);
		console.timeEnd('netClustering');

		data.nodes = data.nodes.filter(function(element,index,array){
			return element.cluster == 1;
		});

		data.links = data.links.filter(function(element,index,array){
			return data.nodes.includes(element.source) && data.nodes.includes(element.target);
		});
		    	...
		    	d3.select(this).selectAll('.rect').each(function(s,i){
		    		d3.select(this).attr('x',center.x).attr('y',center.y)
		    			.attr('transform',function(){
			    			degree = -degrees+90+(60/(preferences['seq_size']+2))*(i+1);
